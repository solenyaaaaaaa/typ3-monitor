...
**Email delivery hardening (added 2026-06-19):** `email_alerts()` now retries the SMTP send up to 3 times (`email_retry_attempts`, exponential backoff) before giving up, so a transient Gmail/network hiccup no longer drops a run's alerts. A live `test-email` workflow run on 2026-06-19 confirmed end-to-end delivery to `EMAIL_TO`. **Residual risk:** a *persistent* failure (e.g. a revoked App Password) still loses alerts after the 3 retries, because `main()` saves state regardless of delivery; this is logged loudly (`email send FAILED after N attempts`) but not yet self-healing. Historical note: a product matched during local development and committed into the initial `state.json` (Five Leaf "Super Boof") was never emailed, because the live cloud monitor saw it already in the seed and never treated it as new.

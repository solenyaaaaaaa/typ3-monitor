...
**Known gap (not yet fixed):** in `main()`, `state.json` is saved even when `email_alerts()` raised, so a transient SMTP failure drops that run's alerts permanently (email is best-effort with no retry, and the advanced state then suppresses re-alerting). Separately, a product matched during local development and committed into the initial `state.json` (e.g. Five Leaf "Super Boof") was never emailed, because the live cloud monitor saw it already in the seed and never treated it as new.
